...
interface Observation {
  ...
  sourceAdapter?: string; // ECS source-tagging: which adapter produced it ("pdf:mistral","sqlite","exif","c2pa","cv-detection",…)
...
**CV pre-pass (Phase 2, `src/core/cv/ObjectDetectionService.ts`).** Opt-in, **signal-not-verdict** (the tool never concludes; the model + human judge). **2a object detection (BUILT):** `ObjectDetectionService` runs a detector via `@huggingface/transformers` (already a dep; bundles onnxruntime-node + sharp — **no new dep**), config-selectable `closed` (`object-detection`, COCO-80; DETR/YOLOS) or `zero-shot` (`zero-shot-object-detection`, open-vocab via `readers.cv.detection.labels`; OWL-ViT). It's a **singleton** (loads the model once), lazy-imported, **injected into `ImageReader`**, and never throws (any failure → `[]`, the C2PA `unavailable` discipline). Detections feed BOTH sinks: the **VLM** (a `CV pre-pass detected: …` line appended to the chunk `content`, which reaches the vision model verbatim) **and** the **graph** (`buildImageMetaGraph` → an `object` entity + `depicts` edge per label, `sourceAdapter:"cv-detection"`, confidence = the detector's score). Config `readers.cv.detection {enabled(false), mode, model, threshold, labels, maxObjects, cacheDir?, allowRemote}`; flags `--object-detection`/`--detection-mode`; default OFF ⇒ byte-identical. **2b forensic signals (DESIGNED, not built):** ELA/JPEG-ghost/copy-move/noise via a vendored OpenCV/Python subproject (the `audio-pipeline` `uv run` shell pattern) → **confidence-floored, hedged, uninterpreted** `sourceAdapter:"cv-forensics"` observations — *never a verdict; a signal that can't be honestly hedged is cut* ("worse than silence") — gated on 2a validation + a corpus. All views (EXIF · C2PA · cv-detection · cv-forensics · the VLM's own read) are `sourceAdapter`-tagged on one image node; active cross-view reconciliation is a later refinement.
...
| `EpubReader` | `.epub` | `adm-zip` + `cheerio` (xmlMode) + `html-to-text` |
| `JupyterReader` | `.ipynb` | Built-in (JSON; reuses MarkdownReader image path) |
...
| `ImageReader` | `.jpg`, `.png`, `.gif`, `.webp`, etc. | Vision model via Ollama (+ opt-in `exifr` EXIF / `c2patool` C2PA / `@huggingface/transformers` object-detection enrichment → graph facts + VLM context) |
