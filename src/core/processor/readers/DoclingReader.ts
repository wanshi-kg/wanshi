...
export class DoclingReader extends FileReader {
  ...
  constructor(
    ...
    super([
      // Document formats
      ".pdf", ".docx", ".xlsx", ".pptx", ".odt", ".odp", ".ods",
      // Web formats  
      ".html", ".htm", ".md", ".txt", ".rtf",
  ...
  async read(filePath: string): Promise<FileReadResult> {
    ...
    try {
      logger.debug(`Processing document with Docling: ${filePath}`);
      ...
      logger.debug(`Successfully processed ${filePath} with Docling in ${processingTime}ms`);
      ...
      logger.error(`Failed to process document with Docling ${filePath}: ${error.message}`);
   ...
   */
  private async processWithDocling(filePath: string): Promise<DoclingProcessResult> {
    const outputPath = path.join(
      this.tempDir,
      `docling_output_${Date.now()}_${Math.random().toString(36).slice(2, 11)}.json`
    );

    try {
      // Build Docling command
      const doclingArgs = [
        "-m", "docling.cli.main",  // Use CLI module
        filePath,                  // Input file
        "--format", "json",        // Output as JSON for structured parsing
        "--output", outputPath,    // Output file
        "--max-pages", this.maxPages.toString(),
        "--max-file-size", this.maxFileSize.toString()
      ];

      logger.debug(`Running Docling: ${this.pythonExecutable} ${doclingArgs.join(" ")}`);

      // Execute Docling
      const result = await this.executeCommand(this.pythonExecutable, doclingArgs);
      ...
      const processedResult = this.parseDoclingOutput(doclingOutput, result.stdout);
   ...
   */
  private parseDoclingOutput(doclingJson: any, stdout: string): DoclingProcessResult {
    try {
      ...
      const metadata: any = {
        ...
        creationDate: doclingJson.creation_date || doclingJson.metadata?.creation_date || "",
        modificationDate: doclingJson.modification_date || doclingJson.metadata?.modification_date || "",
   ...
   */
  private async extractImages(doclingResult: DoclingProcessResult): Promise<Buffer[]> {
    const images: Buffer[] = [];
    ...
    try {
      if (doclingResult.images && Array.isArray(doclingResult.images)) {
        for (const imageInfo of doclingResult.images) {
          if (imageInfo.data) {
            ...
            if (typeof imageInfo.data === "string" && imageInfo.data.startsWith("data:")) {
              const base64Data = imageInfo.data.split(",")[1];
              const buffer = Buffer.from(base64Data, "base64");
              images.push(buffer);
            } else if (imageInfo.path && await this.fileExists(imageInfo.path)) {
      ...
      logger.warn(`Could not extract images from Docling output: ${error}`);
    }
  ...
  private async cleanup(filePath: string): Promise<void> {
    try {
      ...
      logger.warn(`Could not clean up temporary file ${filePath}: ${error}`);
    }
  }
