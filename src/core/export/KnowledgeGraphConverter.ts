...
import { DotExportOptions } from "../../types";
...
export class KnowledgeGraphConverter {
  ...
  static toDOT(graph: KnowledgeGraph, options: DotExportOptions = {}): string {
    const {
      layout = 'dot',
      rankdir = 'TB',
      nodeShape = 'box',
    ...
    if (clusterByFile) {
      const files = [...new Set(graph.entities.map(e => e.files[0]).filter(Boolean))];
    ...
    if (clusterByEntityType && !clusterByFile) {
      entityTypes.forEach((entityType, index) => {
        ...
        if (entitiesOfType.length > 1) {
          ...
          lines.push(`    color="${colors.clusterColors[index % colors.clusterColors.length]}";`);
    ...
    if (clusterByFile) {
      for (const [file, clusterId] of fileClusterMap) {
        const entitiesInFile = graph.entities.filter(e => e.files[0] === file);
    ...
    graph.entities.forEach(entity => {
      ...
      if (includeObservations && entity.observations && entity.observations.length > 0) {
        const observations = entity.observations.slice(0, maxObservationsPerNode);
        ...
        if (entity.observations.length > maxObservationsPerNode) {
          label += `\\n... +${entity.observations.length - maxObservationsPerNode} more`;
        }
      }
      ...
      lines.push(`  ${nodeId} [label="${label}", fillcolor="${color}", tooltip="${this.escapeLabel(entity.entityType)}"];`);
    });
    ...
    uniqueRelationTypes.forEach((relType, index) => {
      relationColorMap.set(relType, colors.relationColors[index % colors.relationColors.length]);
    });
    ...
    graph.relations.forEach(relation => {
      ...
      relationTypes.forEach(relType => {
        ...
        lines.push(`  ${fromId} -> ${toId} [label="${label}", color="${color}", fontcolor="${color}"];`);
      });
    });
    ...
    if (showLegend) {
      ...
      entityTypes.forEach((entityType, index) => {
        ...
        lines.push(`    ${legendNodeId} [label="${this.escapeLabel(entityType)}", shape="box", fillcolor="${color}"];`);
      });
      ...
      if (uniqueRelationTypes.length > 0) {
        ...
        lines.push('    legend_rel_start [label="Relations:", shape="plaintext"];');
        
        uniqueRelationTypes.slice(0, 10).forEach((relType, index) => { // Limit to 10 for readability
          const color = relationColorMap.get(relType) || colors.defaultRelationColor;
          const legendNodeId = `legend_rel_${index}`;
          lines.push(`    legend_rel_${index}_from [label="", shape="point", width="0.1"];`);
          lines.push(`    legend_rel_${index}_to [label="${this.escapeLabel(relType)}", shape="plaintext"];`);
          lines.push(`    legend_rel_${index}_from -> legend_rel_${index}_to [color="${color}", fontcolor="${color}"];`);
        });
      }
  ...
  private static getColorScheme(scheme: string) {
    switch (scheme) {
      case 'scientific':
        return {
          entityColors: ['lightblue', 'lightgreen', 'lightyellow', 'lightcoral', 'lightpink', 'lightgray', 'lightsteelblue', 'lightseagreen'],
          relationColors: ['blue', 'darkgreen', 'orange', 'red', 'purple', 'brown', 'darkblue', 'darkred'],
      ...
      case 'code':
        return {
          entityColors: ['#FFE6CC', '#E6F3FF', '#E6FFE6', '#FFE6F3', '#F3E6FF', '#FFFFE6', '#E6FFFF', '#FFE6E6'],
          relationColors: ['#FF6B35', '#004E89', '#4CAF50', '#E91E63', '#9C27B0', '#FF9800', '#00BCD4', '#F44336'],
  ...
  // Utility method to generate DOT with ts-graphviz integration
  static async generateGraphvizImage(
    ...
    try {
      // This requires ts-graphviz to be installed
      const { toDot } = await import('ts-graphviz');
      
      const dotContent = this.toDOT(graph, options);
      
      // Save DOT file
      const dotPath = outputPath.replace(/\.[^.]+$/, '.dot');
      require('fs').writeFileSync(dotPath, dotContent);
      
      logger.info(`DOT file saved to: ${dotPath}`);
      logger.info(`You can visualize with: dot -T${format} ${dotPath} -o ${outputPath}`);
      logger.info(`Or use online: https://dreampuf.github.io/GraphvizOnline/`);
      
    } catch (error) {
      logger.error(`Failed to generate Graphviz image: ${error}`);
      logger.info('Install ts-graphviz for image generation: npm install ts-graphviz');
      
      // Fallback: just save the DOT content
      const dotPath = outputPath.replace(/\.[^.]+$/, '.dot');
      require('fs').writeFileSync(dotPath, this.toDOT(graph, options));
      logger.info(`DOT file saved to: ${dotPath}`);
    }
  }
}